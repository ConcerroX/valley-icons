...
object FilePathManager {
    ...
    private const val DESKTOP_DIRECTORY = "Desktop"
    ...
    fun getDesktopImageFile(fileName: String): File? {
        val userHome = System.getProperty("user.home")
        if (userHome.isNullOrBlank()) {
            return null
        }

        val imageFile = File(File(userHome, DESKTOP_DIRECTORY), fileName)
        return imageFile.takeIf { it.exists() && it.isFile }
    }
